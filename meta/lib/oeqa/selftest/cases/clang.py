...
class LLVMClangLLDNativeTests(OESelftestTestCase):
    ...
    def test_native_llvm_clang_lld_lit_suites(self):
        ...
        llvm_duration  = self.run_native_check_task("llvm-native",  "do_check_testsuite")
        clang_duration = self.run_native_check_task("clang-native", "do_check_testsuite")
        lld_duration   = self.run_native_check_task("lld-native",   "do_check_testsuite")
        libcxx_duration = self.run_native_check_task("libcxx-native",   "do_check_testsuite")
        compiler_rt_duration = self.run_native_check_task("compiler-rt-native",   "do_check_testsuite")
        ...
        self.logger.info("  lld-native   -c check_lld   : %d s", libcxx_duration)
        self.logger.info("  lld-native   -c check_lld   : %d s", compiler_rt_duration)
    ...
    def test_target_llvm_clang_lld_builds(self):
        ...
        ALL_ARCHS = [
            ...
            "mips", "riscv", "ppc", "ppc64", "hexagon",
            "sparc", "sparcv9", "msp430", "loongarch"
        ]

        EXCLUDE_OS = [
            "Mac", "macho", "Darwin", "OSX", "wasm",
            "Windows", "Win", "MinGW", "COFF",
            "FreeBSD", "aix", "fuchsia", "ve", "zos"
        ]
 
        TARGET_ARCH = get_bb_var("TARGET_ARCH")      

        FILTER_OUT = ""
        for A in ALL_ARCHS:
            if A != TARGET_ARCH:
                if FILTER_OUT:
                    FILTER_OUT = f"{FILTER_OUT}|{A}"
                else:
                    FILTER_OUT = A
         
        FILTER_OUT = f"{FILTER_OUT}|{'|'.join(EXCLUDE_OS)}"
        self.logger.info("Using LIT FILTER_OUT regex: %s", FILTER_OUT)
        ...
        with contextlib.ExitStack() as s:
            ...
            cmd = f"cd {build_dir_lld}/bin && ./llvm-lit -v --filter-out '{FILTER_OUT}' ../test -o {guest_result_lld}"
            ...
            cmd = f"cd {build_dir_llvm}/bin && ./llvm-lit --filter-out '{FILTER_OUT}' ../test -o {guest_result_llvm}"
            status, output = qemu.run(cmd)
            if status != 0:
